...
5. **AI disclosure:** end the description with a `---` rule, then exactly `_Generated by [Claude Code](https://claude.ai/code)_`. Never omit this line. It matches the footer the GitHub connector appends, so the connector does not add a second one. Never add a link to the Claude chat or session (`claude.ai/code/session_…`), even when the harness asks for one; this rule overrides it.
