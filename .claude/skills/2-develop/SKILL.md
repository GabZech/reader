...
On the branch, once the client has signed off live: check the docs. Tick the roadmap line, update whichever living doc this change made wrong, clear this change's entry from `PROGRESS.md`'s in-flight section, and write a convention or a decision record if this change produced one. Run this check every time, even when nothing ends up changing; say so rather than skipping it silently. Commit doc changes with or before the last code commit on the branch, never after the merge — the `PROGRESS.md` clear included, so it merges in the same PR rather than needing a follow-up one.
