...
Clarifying **content** for Scope. Deliver each ask with the questioning skill (`.claude/skills/questioning/SKILL.md`) — style lives there; this file is only the menu.

Pick and adapt; do not read as a script. Reuse Understand answers; decide, do not restart.

Epics here are **named capabilities** at a coarser level of detail than Build increments. Each traces to at least one accepted journey. Increments wait for Plan; screens wait for Mockup; stack waits for Foundation.
