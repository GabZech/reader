...
- **Repo maintenance** (docs, skills, CI, this file): the change loop's gates (Preview, live sign-off, deploy) do not apply, but it still commits incrementally, still merges only through a PR opened with the client's explicit go-ahead (the client merges it, or explicitly tells the agent to), and still routes anything touched through the table below.
...
3. **Sign-off gates merge, not commit; merge always goes through a PR; opening that PR needs the client's explicit go-ahead.** Commit on the branch as checkpoints pass. Once the client signs off live, ask before opening the PR; only then does Ship open it instead of pushing to `main` directly. The client merges it, or explicitly tells the agent to.
