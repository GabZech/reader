...
- **List membership.** Manual author-name or feed-ID query editing is not an acceptable standing workflow. A list groups items for a moment; it is not a single media type. The lists in the picture now are News, Read later, and Favourite channels. News and Favourite channels group sources. Read later holds articles parked for a later pass. More lists will appear later, including mixed YouTube plus newsletter/RSS lists. The replacement path is **Wanted** on [Add a Source to a List](journeys.md#add-a-source-to-a-list).
