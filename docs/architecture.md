...
**Proved (this slice):** the shell (Home, Lists, Sources), SQLite, a public RSS fetch on sync that keeps only the latest five items from the demo feed, item reading, PWA cache registration, tests, local run. Hosted deploy is the intended run path and is not yet smoked: Docker and a Fly account were not available on the machine that built the skeleton.

**Still to stand up in the first feature and after:** isolated-mailbox IMAP; adding a source to a list; article cleanup on open for linked pages; vault markdown; list edit, home edit, highlighting, and the rest of first-product behaviour.
