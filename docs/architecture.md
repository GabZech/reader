...
**Still to stand up in the first feature and after:** isolated-mailbox IMAP; adding a source to a list; article cleanup on open for linked pages (the demo RSS often ships a teaser, not the full page); vault markdown; list edit, home edit, highlighting, and the rest of first-product behaviour. The lasting GitHub repo name, Fly URL, and home-screen logo wait until that first product is about to go on the phone; until then the host stays `reader-skeleton` and the install icon stays the skeleton SVG.
