...
- The test suite has no speed tiering: all 280 tests (browser-driven ones included) run together every time. Currently ~60s, not yet a problem. Revisit once the full run noticeably slows down local iteration: split fast unit tests from browser tests (pytest markers) and consider parallelizing (pytest-xdist)
